...
var TinyMCE_SimpleTheme = {
	...
	_buttonMap : 'anchor,backcolor,bold,bullist,charmap,cleanup,code,forecolor,help,hr,image,indent,italic,justifycenter,justifyfull,justifyleft,justifyright,link,numlist,outdent,redo,sub,sup,underline,undo,unlink,visualaid,ltr,rtl,flash,selectall,preview,table,cell_props,delete_col,delete_row,col_after,col_before,row_after,row_before,merge_cells,row_props,split_cells',
	...
	getControlHTML : function(button_name) {
		...
		for (i=0; i<TinyMCE_SimpleTheme._buttons.length; i++) {
			var but = TinyMCE_SimpleTheme._buttons[i];
		...
		switch (button_name) {
			case "formatselect":
				...
				var formats = tinyMCE.getParam("theme_simple_blockformats", "p,address,pre,h1,h2,h3,h4,h5,h6", true).split(',');
				var lookup = [
			...
			case "fontselect":
				...
				var iFonts = '宋体=宋体;黑体=黑体;幼圆=幼圆;隶书=隶书;楷体=楷体;Arial=arial,helvetica,sans-serif;Courier New=courier new,courier,monospace;Georgia=georgia,times new roman,times,serif;Tahoma=tahoma,arial,helvetica,sans-serif;Times New Roman=times new roman,times,serif;Verdana=verdana,arial,helvetica,sans-serif;Impact=impact;WingDings=wingdings';
				var nFonts = '宋体=宋体;黑体=黑体;幼圆=幼圆;隶书=隶书;楷体=楷体;Andale Mono=andale mono,times;Arial=arial,helvetica,sans-serif;Arial Black=arial black,avant garde;Book Antiqua=book antiqua,palatino;Comic Sans MS=comic sans ms,sand;Courier New=courier new,courier;Georgia=georgia,palatino;Helvetica=helvetica;Impact=impact,chicago;Symbol=symbol;Tahoma=tahoma,arial,helvetica,sans-serif;Terminal=terminal,monaco;Times New Roman=times new roman,times;Trebuchet MS=trebuchet ms,geneva;Verdana=verdana,geneva;Webdings=webdings;Wingdings=wingdings,zapf dingbats';
				var fonts = tinyMCE.getParam("theme_simple_fonts", iFonts).split(';');
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		switch (command) {
			...
			case "mceLink":
				...
				if (inst.settings['insertlink_callback']) {
					...
					if (returnVal && returnVal['href'])
						TinyMCE_SimpleTheme._insertLink(returnVal['href'], returnVal['target'], returnVal['title'], returnVal['onclick'], returnVal['style_class']);
				} else {
			...
			case "mceImage":
				...
				if (inst.settings['insertimage_callback']) {
					...
					if (returnVal && returnVal['src'])
						TinyMCE_SimpleTheme._insertImage(returnVal['src'], returnVal['alt'], returnVal['border'], returnVal['hspace'], returnVal['vspace'], returnVal['width'], returnVal['height'], returnVal['align'], returnVal['title'], returnVal['onmouseover'], returnVal['onmouseout']);
				} else
			...
			case "mceBackColor":
				...
				template['width'] += tinyMCE.getLang('lang_theme_simple_backcolor_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_simple_backcolor_delta_height', 0);
			...
			case "mceCodeEditor":
				...
				template['width'] = parseInt(tinyMCE.getParam("theme_simple_source_editor_width", 720));
				template['height'] = parseInt(tinyMCE.getParam("theme_simple_source_editor_height", 580));
			...
			case "mceCharMap":
				...
				template['width'] += tinyMCE.getLang('lang_theme_simple_charmap_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_simple_charmap_delta_height', 0);
			...
			case "mceInsertAnchor":
				...
				template['width'] += tinyMCE.getLang('lang_theme_simple_anchor_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_simple_anchor_delta_height', 0);
	...
	getEditorTemplate : function(settings, editorId) {
		...
		var template = new Array();
		var deltaHeight = 0;
		var resizing = tinyMCE.getParam("theme_simple_resizing", false);
		var path = tinyMCE.getParam("theme_simple_path", true);
		var statusbarHTML = '<div id="{$editor_id}_path" class="mceStatusbarPathText" style="display: ' + (path ? "block" : "none") + '">&nbsp;</div><div id="{$editor_id}_resize" class="mceStatusbarResize" style="display: ' + (resizing ? "block" : "none") + '" onmousedown="TinyMCE_SimpleTheme._setResizing(event,\'{$editor_id}\',true);"></div><br style="clear: both" />';
		var layoutManager = tinyMCE.getParam("theme_simple_layout_manager", "SimpleLayout");

		switch(layoutManager) {
			case "SimpleLayout" : //the default TinyMCE Layout (for backwards compatibility)...
				var toolbarHTML = "";
				var toolbarLocation = tinyMCE.getParam("theme_simple_toolbar_location", "top");
				var pathLocation = tinyMCE.getParam("theme_simple_path_location", "bottom"); // Compatiblity
				var statusbarLocation = tinyMCE.getParam("theme_simple_statusbar_location", pathLocation);
				var defVals = {
					theme_simple_buttons1 : "formatselect,fontselect,fontsizeselect,cut,copy,paste,separator,pastetext,pasteword,selectall,separator,bold,italic,underline,separator,justifyleft,justifycenter,justifyright,justifyfull,bullist,numlist,separator,outdent,indent,separator",
					theme_simple_buttons2 : "undo,redo,separator,hr,link,unlink,anchor,image,flash,cleanup,forecolor,backcolor,separator,tablecontrols,separator,visualaid,separator,sub,sup,charmap,separator,preview,code"
				};
				...
				// Render rows
				for (var i=1; i<100; i++) {
					var def = defVals["theme_simple_buttons" + i];

					var buttons = tinyMCE.getParam("theme_simple_buttons" + i, def == null ? '' : def, true, ',');
					if (buttons.length == 0)
						break;

					buttons = removeFromArray(buttons, tinyMCE.getParam("theme_simple_disable", "", true, ','));
					buttons = addToArray(buttons, tinyMCE.getParam("theme_simple_buttons" + i + "_add", "", true, ','));
					buttons = addToArray(tinyMCE.getParam("theme_simple_buttons" + i + "_add_before", "", true, ','), buttons);
				...
				if (toolbarLocation == "top") {
					template['html'] += '<tr><td class="mceToolbarTop" height="1" nowrap="nowrap">' + toolbarHTML + '</td></tr>';
				}

				template['html'] += '<tr><td align="center"><span id="{$editor_id}"></span></td></tr>';

				if (toolbarLocation == "bottom") {
					template['html'] += '<tr><td class="mceToolbarBottom" height="1">' + toolbarHTML + '</td></tr>';
				}
			...
			case "RowLayout" : //Container Layout - containers defined in "theme_simple_containers" are rendered from top to bottom.
				template['html'] = '<table class="mceEditor" border="0" cellpadding="0" cellspacing="0" width="{$width}" height="{$height}" style="width:{$width}px;height:{$height}px"><tbody>';

				var containers = tinyMCE.getParam("theme_simple_containers", "", true, ",");
				var defaultContainerCSS = tinyMCE.getParam("theme_simple_containers_default_class", "container");
				var defaultContainerAlign = tinyMCE.getParam("theme_simple_containers_default_align", "center");
				...
				for (var i = 0; i < containers.length; i++)
				{
					...
					else if (containers[i] == "mceElementpath" || containers[i] == "mceStatusbar") // ... mceElementpath:
					{
						...
						var curContainer = tinyMCE.getParam("theme_simple_container_"+containers[i], "", true, ',');
						var curContainerHTML = "";
						var curAlign = tinyMCE.getParam("theme_simple_container_"+containers[i]+"_align", defaultContainerAlign);
						var curCSS = tinyMCE.getParam("theme_simple_container_"+containers[i]+"_class", defaultContainerCSS);
			...
			case "CustomLayout" : //User defined layout callback...
				var customLayout = tinyMCE.getParam("theme_simple_custom_layout","");
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection, setup_content) {
		...
		if (selectElm) {
			TinyMCE_SimpleTheme._setupCSSClasses(editor_id);
	...
	_setupCSSClasses : function(editor_id) {
		if (!TinyMCE_SimpleTheme._autoImportCSSClasses)	{
			return;
		}
	...
	_resizeEventHandler : function(e) {
		var resizer = TinyMCE_SimpleTheme._resizer;
		...
		switch (e.type) {
			...
			case "mouseup":
				TinyMCE_SimpleTheme._setResizing(e, editorId, false);
				TinyMCE_SimpleTheme._resizeTo(tinyMCE.getInstanceById(editorId), resizer.width + dx, resizer.height + dy, resizer.horizontal);
				...
				if (tinyMCE.getParam("theme_simple_resizing_use_cookie", true)) {
					...
					// Set the cookies
					TinyMCE_SimpleTheme._setCookie("TinyMCE_" + editorId + "_width", "" + (resizer.horizontal ? resizer.width + dx : ""), expires);
					TinyMCE_SimpleTheme._setCookie("TinyMCE_" + editorId + "_height", "" + (resizer.height + dy), expires);
				}
				break;
		}
	},
	...
	_setResizing : function(e, editor_id, state) {
		...
		if (state) {
			...
			if (!resizer.eventHandlers) {
				if (tinyMCE.isMSIE)
					tinyMCE.addEvent(document, "mousemove", TinyMCE_SimpleTheme._resizeEventHandler);
				else
					tinyMCE.addEvent(window, "mousemove", TinyMCE_SimpleTheme._resizeEventHandler);

				tinyMCE.addEvent(document, "mouseup", TinyMCE_SimpleTheme._resizeEventHandler);
			...
			resizer.horizontal = tinyMCE.getParam("theme_simple_resize_horizontal", true);
		} else {
...
function  cleanWordString(str){ 
	   ...
	   html  =  html.replace(/<(v:imagedata[^>]*) src=([^ |>]*)([^>]*)/gi,"<img src=$2");
           html  =  html.replace(/<\/?\w+:[^>]*>/gi,"");
      return html;
}  

function  cleanWordImg(str){ 
	   var html = str;
           html  =  html.replace(/<(\w[^>]*) class=([^ |>]*)([^>]*)/gi,"<$1$3"); 
           //html  =  html.replace(/<(\w[^>]*) style="([^"]*)"([^>]*)/gi,"<$1$3");
	   html  =  html.replace(/mso\-([^;|"]*)/gi,"");
...
// Add default buttons maps for simple theme and all internal plugins
