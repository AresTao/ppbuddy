	...
	_setHTML : function(doc, html_content) {
		...
		try {
			...
			if (this.isMSIE){
				...
				try{doc.body.createTextRange().pasteHTML(html_content);}catch(e){}
					...
					for (var i=0; i<csses.length; i++) {
						...
						if (selectorText) {
							...
							for (var c=0; c<rules.length; c++) {
								...
								if (rules[c] == "." + tinyMCE.settings['visual_table_class'] || rules[c].indexOf('mceEditable') != -1)
									continue;
