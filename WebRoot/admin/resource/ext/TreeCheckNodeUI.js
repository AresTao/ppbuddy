 ...
 * 
 * 使用方法：在loader里加上 baseAttrs:{uiProvider:Ext.ux.TreeCheckNodeUI} 既可. 例如： var
 ...
 * Ext.ux.TreeCheckNodeUI } //添加 uiProvider 属性 }), root: new
...
Ext.extend(Ext.ux.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
	...
	check : function(checked) {
		...
		if (this.checkModel == 'single') {
			...
			if (this.checkModel == 'prePCCascade') {
				var parentNode = n.parentNode;
				if (parentNode !== null) {
					this.preParentCheck(parentNode, checked);
				}
			}
			if (this.checkModel == 'cascade'
					|| this.checkModel == 'childCascade'
					|| this.checkModel == 'prePCCascade') {
	...
	// 有条件的判断其父节点应该选中还是不选中
	preParentCheck : function(node, checked) {
		var checkbox = node.getUI().checkbox;
		// 判断当前节点是不是全部选中
		if (!this.childAllChecked(node)) {
			if (typeof checkbox == 'undefined') {
				return;
			}
			if (checked && !this.childAllChecked(node)) {
				return;
			}
