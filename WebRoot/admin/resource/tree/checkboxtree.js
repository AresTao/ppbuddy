...
function createTree(arrName, startNode, openNode, chkboxName) {
    ...
    if (nodes.length > 0) {
        ...
        if (rootidex == -1) {//生成默认的根节点
            nodes[nodes.length]="0||root||true";
            rootidex=nodes.length-1;
        }
...
//        if (rootidex != -1) {
            ...
            add(nodeValues[0]);
            document.write("<input type='checkbox' id='" + parent + "' name='" + chkboxName + "' " + checked_value(nodeValues[4]) + " value='" + nodeValues[0] + "' onclick='checkSelect(this,\"" + nodeValues[0] + "\")'><img src=\"../resource/images/tree/base.gif\" align=\"absbottom\" />" + nodeValues[2] + "<br />");
//        } else {
//            alert("warn2!");
//        }
...
function setOpenNodes(openIndex) {
    openNode=nodes[openIndex];
    for (i = 0; i < nodes.length; i++) {
        var nodeValues = nodes[i].split("|");
...
function addNode(parentNode, recursedNodes, chkboxName) {
    for (var i = 0; i < nodes.length; i++) {
        ...
        if(nodeValues[1]==""||nodeValues[1]=="null"){
            nodeValues[1]=0;//修改没有根的节点到默认根节点
        }
        ...
        if (nodeValues[1] == parentNode && rootidex != i) {
            ...
            for (g = 0; g < recursedNodes.length; g++) {
                if (recursedNodes[g] == 1) {
                    document.write("<img src=\"../resource/images/tree/line.gif\" align=\"absbottom\" alt=\"\" />");
                } else {
                    document.write("<img src=\"../resource/images/tree/empty.gif\" align=\"absbottom\" alt=\"\" />");
                }
            }
            ...
            if (hcn) {
                document.write("<input type='checkbox' id='" + parent + "' name='" + chkboxName + "' value='" + nodeValues[0] + "' " + checked_value(nodeValues[4]) + " onclick='checkSelect(this,\"" + nodeValues[0] + "\")'><img id=\"icon" + nodeValues[0] + "\" src=\"../resource/images/tree/folder");
                if (ino) {
                    document.write("open");
                }
                document.write(".gif\" align=\"absbottom\"  />");
            } else {
                document.write("<input type='checkbox' id='" + parent + "' name='" + chkboxName + "' value='" + nodeValues[0] + "' " + checked_value(nodeValues[4]) + " onclick='checkSelect(this,\"" + nodeValues[0] + "\")'>");
            }
